...
fn build_application() -> Application {
    ...
    if std::env::var("CLAY_EXPERIMENTAL_A11Y").as_deref() == Ok("1") {
...
fn main() {
    ...
    match util::get_zed_cli_path() {
        ...
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("CLAY_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    let version = option_env!("CLAY_BUILD_ID");
    let app_commit_sha =
        option_env!("CLAY_COMMIT_SHA").map(|commit_sha| AppCommitSha::new(commit_sha.to_string()));
    ...
    let git_binary_path =
        if cfg!(target_os = "macos") && option_env!("CLAY_BUNDLE").as_deref() == Some("true") {
    ...
    if let Some(kind) = request.kind {
        match kind {
                    ...
                    multi_workspace.update(cx, |multi_workspace, window, cx| {
                        multi_workspace.workspace().update(cx, |workspace, cx| {
                            if let Some(panel) = workspace.focus_panel::<AgentPanel>(window, cx) {
                                ...
                                log::warn!(
                                    "clay://agent received but the AgentPanel is not registered \
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("clay://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                // clay://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // clay://settings/languages/Rust/tab_size  - SUPPORT
...
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `clay://` scheme, or relative to <https://zed.dev>.
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                || arg.starts_with("clay://")
                || arg.starts_with("clay-cli://")
                || arg.starts_with("ssh://")
