use crate::session;
use gpui::{
    Context, EventEmitter, IntoElement, MouseButton, ParentElement, Render, SharedString, Styled,
...
impl BookmarkBar {
    ...
    fn open_bar_context_menu(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        // TODO: right-click menus are not ported yet. Glass used an AppKit popup whose
        // callback dispatches on an item index; `ui::ContextMenu` takes a handler per entry
        // and needs menu state plus a dismiss subscription on this view, so it is a real
        // restructure rather than a translation. Left out so the browser can be exercised.
        let _ = (is_always, position, window, view);
    }
                ...
                move |(url, title, favicon_url)| {
                    ...
                    div()
                        ...
                        .on_mouse_down(MouseButton::Right, move |_event, _window, _cx| {
                            // TODO: see the note above; the per-bookmark menu offered
                            // "Open in New Tab" and "Delete".
                            let _ = (&new_tab_url_for_menu, &delete_url_for_menu, &view_for_menu);
                        })
                        .when_some(favicon_element, |this, f| this.child(f))
                        .when(favicon_url.is_none(), |this| {
                            this.child(
                                Icon::new(IconName::Public).size(IconSize::Small),
                            )
                        })
                        .child(
                            div()
