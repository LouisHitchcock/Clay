...
fn convert_key_event(keystroke: &Keystroke, is_down: bool) -> KeyEvent {
    ...
    // Glass carried a macOS virtual keycode on `Keystroke`; upstream GPUI has no such field
    // on any platform, so the key name is the only source. `key_name_to_windows_vk` already
    // produces what CEF wants, and CEF only needs the raw scan code for legacy IME paths.
    let _ = prefer_logical_key;
    let windows_key_code = key_name_to_windows_vk(&keystroke.key);
    let native_key_code = 0;
