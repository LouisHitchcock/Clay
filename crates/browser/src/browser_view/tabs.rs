...
impl BrowserView {
    pub(crate) fn add_tab(&mut self, cx: &mut Context<Self>) {
        ...
        // A new tab lands on the new tab page, whose own search box is the only place to type
        // a URL — the toolbar omnibox is hidden there.
    ...
    /// Start pumping CEF's message loop, once CEF is ready for it.
    ///
    /// Deliberately creates no browser. A windowless browser created before CEF's message
    /// loop has run never begins painting — `on_accelerated_paint` simply never fires for it,
    /// even though `create_browser` succeeds and the load starts. That is what left a restored
    /// session showing a blank page: it was the one path that created a browser before the
    /// pump existed. `ensure_active_tab_browser` does the creating, on a later pass.
    pub(super) fn start_message_pump_if_ready(&mut self, cx: &mut Context<Self>) {
        if self.message_pump_started || !CefInstance::is_context_ready() {
            return;
        }
    ...
    /// Give the active tab a CEF browser if it has not got one yet.
    ///
    /// Restored tabs arrive with a URL and no browser, so this is what loads a session on
    /// startup, and what loads any restored tab the first time it comes to the front.
    pub(super) fn ensure_active_tab_browser(
        &mut self,
        width: u32,
        height: u32,
        scale_factor: f32,
        ...
        let url = {
            let tab = tab.read(cx);
            let wants_browser = !tab.has_browser()
                && !tab.is_new_tab_page()
                && !tab.is_suspended()
                && !tab.url().is_empty();
            if !wants_browser {
        ...
        tab.update(cx, |tab, _| {
            ...
            if let Err(error) = tab.create_browser(&url) {
                log::error!("[browser::tabs] failed to create a browser for {url}: {error}");
                return;
            }
            tab.set_focus(true);
            tab.invalidate();
