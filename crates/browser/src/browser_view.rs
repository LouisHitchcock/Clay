...
pub struct BrowserView {
    ...
    /// The pane toolbar carrying the omnibox. Owned by the workspace pane, not by this view;
    /// held here only so `FocusOmnibox` has something to focus.
...
impl BrowserView {
    ...
    pub(crate) fn update_toolbar_active_tab(
        ...
        // The toolbar itself observes this view and picks up the active tab on its own, so
        // there is deliberately nothing to push here.
        let _ = window;
        self.sync_bookmark_bar_visibility(cx);
    }

    /// Whether this browser has any tabs at all.
    pub(crate) fn has_tabs(&self) -> bool {
        !self.tabs.is_empty()
    }

    /// Record the toolbar the workspace pane holds for this browser.
    pub(crate) fn set_pane_toolbar(&mut self, toolbar: Entity<BrowserToolbar>) {
...
impl BrowserPaneItem {
    /// The browser this tab shows. Clay can have several browser tabs open at once, so the
    /// toolbar rebinds to whichever one is active rather than assuming a single browser.
    pub fn browser_view(&self) -> &Entity<BrowserView> {
        &self.browser_view
    }
}
...
impl Item for BrowserPaneItem {
    ...
    fn tab_icon(&self, _window: &Window, _cx: &App) -> Option<ui::Icon> {
        Some(ui::Icon::new(ui::IconName::Public))
    }

    fn deactivated(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        {
            self.browser_view.update(cx, |browser_view, cx| {
...
impl Render for BrowserPaneItem {
    fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
        self.browser_view.clone().into_any_element()
    }
}
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if content_width > 0 && content_height > 0 {
            if !self.message_pump_started {
                // Only start the pump on this pass. Creating a browser before CEF's message
                // loop has run leaves it silently never painting, so the active tab gets its
                // browser on the next pass instead — hence the notify either way.
                self.start_message_pump_if_ready(cx);
                cx.notify();
            } else {
                self.ensure_active_tab_browser(content_width, content_height, scale_factor, cx);
