...
fn main() {
    ...
    app.run(move |cx| {
        ...
        if let Ok(raw_flags) = std::env::var("CLAY_EVAL_ENABLE_FLAGS") {
...
const OPENAI_COMPATIBLE_PROVIDERS_ENV: &str = "CLAY_OPENAI_COMPATIBLE_PROVIDERS";
...
const ANTHROPIC_AVAILABLE_MODELS_ENV: &str = "CLAY_ANTHROPIC_AVAILABLE_MODELS";
...
async fn run_agent(
    ...
    let setup_result: Result<()> = cx.update(|cx| {
        ...
        let profile_field = {
            let raw = std::env::var("CLAY_EVAL_DISABLE_TOOLS").unwrap_or_default();
            let disabled = raw
                .split(',')
