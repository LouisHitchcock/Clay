...
/// Outcome of a single account-import invocation.
...
pub fn claude_profiles_dir() -> Option<PathBuf> {
    ...
    Some(util::paths::home_dir().join(".claude-profiles"))
}
...
pub fn import_from_claude_profiles() -> Result<AccountImport> {
    let mut report = AccountImport::default();
...
///
/// cc-switch stores one credentials *file* per account, `<name>.credentials.json`, alongside
/// timestamped `.bak-*` copies, and switches by writing the chosen one over
/// `~/.claude/.credentials.json`.
pub fn cc_switch_accounts_dir() -> PathBuf {
    util::paths::home_dir().join(".claude").join("accounts")
}

/// Registers each cc-switch account as a Clay AI account with its own config directory.
///
/// Unlike [`import_from_claude_profiles`], this cannot register by reference: cc-switch's
/// accounts are credential files that all share the one `~/.claude` config directory, so
/// pointing several accounts at it would make them the same account. Each import therefore gets
/// a managed directory under [`accounts_root`] with the credentials copied in, which is what
/// makes them independently usable through `CLAUDE_CONFIG_DIR`.
///
/// Only the credentials are copied. Everything else in a config directory — settings, projects,
/// session history — starts empty, because those are not account identity and copying them would
/// silently fork the user's configuration.
///
/// The user's own cc-switch setup is left untouched, so both keep working side by side.
pub fn import_from_cc_switch() -> Result<AccountImport> {
    let mut report = AccountImport::default();
    let root = cc_switch_accounts_dir();
    let entries = match std::fs::read_dir(&root) {
        Ok(entries) => entries,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(report),
        Err(error) => return Err(error).context("reading the cc-switch accounts directory"),
    };
    ...
    for entry in entries.flatten() {
        ...
        let Some(name) = cc_switch_account_name(file_name) else {
            continue;
        };

        let already_registered = index
            .for_agent(agent_id)
            .any(|account| account.display_name == name);
        if already_registered {
            report.skipped_existing.push(name.to_string());
            continue;
        }

        let account = AiAccount::new(agent_id, unique_display_name(&index, agent_id, name), {
            let id = uuid::Uuid::new_v4().to_string();
            accounts_root().join(agent_id).join(&id)
        });

        if let Err(error) = seed_config_dir_from_credentials(&account.config_dir, &path) {
            log::error!("ai_accounts: could not import cc-switch account {name}: {error:#}");
...
/// The account name a cc-switch credentials file belongs to, or `None` if the file is not one.
///
/// cc-switch writes timestamped backups next to the real files, e.g.
/// `dronetech.credentials.json.bak-20260829-134302`. Those end past the suffix, so they fall out
/// here rather than being imported as an account called `dronetech.credentials.json.bak-…`.
fn cc_switch_account_name(file_name: &str) -> Option<&str> {
    let name = file_name.strip_suffix(".credentials.json")?;
    (!name.is_empty()).then_some(name)
}

/// Creates `config_dir` and copies `credentials` into it under the name Claude Code reads.
fn seed_config_dir_from_credentials(config_dir: &Path, credentials: &Path) -> Result<()> {
    std::fs::create_dir_all(config_dir)
        .with_context(|| format!("creating {}", config_dir.display()))?;
    std::fs::copy(credentials, config_dir.join(".credentials.json"))
        .with_context(|| format!("copying {}", credentials.display()))?;
    Ok(())
}
...
#[cfg(test)]
mod tests {
    use super::cc_switch_account_name;


    #[test]
    fn cc_switch_credentials_files_name_their_account() {
        assert_eq!(cc_switch_account_name("personal.credentials.json"), Some("personal"));
        assert_eq!(cc_switch_account_name("dronetech.credentials.json"), Some("dronetech"));
    }

    #[test]
    fn cc_switch_backups_and_strays_are_not_accounts() {
        // cc-switch drops timestamped backups in the same directory.
        assert_eq!(
            cc_switch_account_name("dronetech.credentials.json.bak-20260829-134302"),
            None
        );
        assert_eq!(cc_switch_account_name(".credentials.json"), None);
        assert_eq!(cc_switch_account_name("settings.json"), None);
        assert_eq!(cc_switch_account_name("notes.txt"), None);
    }
    use super::*;
